...
export default function HomeLayout() {
  const [isScrolled, setIsScrolled] = useState(false);
  const [lastScrollY, setLastScrollY] = useState(0); // برای ذخیره آخرین مقدار اسکرول

  useEffect(() => {
    const handleScroll = () => {
      const currentScrollY = window.scrollY;

      // اگر اسکرول به پایین باشد و بیشتر از 50 پیکسل باشد
      if (currentScrollY > 50 && currentScrollY > lastScrollY) {
        setIsScrolled(true); // هدر جمع شود
      } else if (currentScrollY < lastScrollY) {
        setIsScrolled(false); // هدر باز شود
      }

      setLastScrollY(currentScrollY); // به‌روزرسانی آخرین مقدار اسکرول
    };
