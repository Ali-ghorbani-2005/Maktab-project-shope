...
const CategoryButtons = () => {
  ...
  return (
    <div className="relative">
      ...
      {/* Slide-out panel */}
      <div
        className={`fixed top-0 right-0 w-full sm:w-72 h-full bg-white shadow-2xl transform transition-transform duration-700 ease-in-out z-40 ${isPanelOpen ? "translate-x-0" : "translate-x-full"}`}
      >
        <div className="p-5">
          {/* Close button */}
          <button
          ...
          {categories.map((category) => (
            ...
            >
              ...
              {/* Subcategories - Slide from the left */}
              {isHovered === category._id && (
                <div className={`absolute right-full top-0 w-64 bg-gray-50 h-auto shadow-xl border border-gray-200 rounded-lg p-2 mr-4 transition-opacity duration-500 ease-in-out`}>
                  {subcategories[category._id]?.length > 0 ? (
                    subcategories[category._id].map((subcategory) => (
                      <button
                        key={subcategory._id}
