...
//         </div>
//         <h2 className="text-2xl font-semibold mb-6">اضافه کردن محصول جدید</h2>
...
//           {/* سایر فیلدها */}
//           {[
//             { name: "name", label: "نام محصول", type: "text" },
//             { name: "price", label: "قیمت", type: "number" },
//             { name: "quantity", label: "تعداد", type: "number" },
...
//           <div className="mb-4">
//             <h3 className="text-lg font-semibold mb-2">پیش‌نمایش تصاویر:</h3>
//             <div className="flex overflow-x-auto">
//               {imagePreviews.map((imgUrl, index) => (
//                 <img key={index} src={imgUrl} alt={`Preview ${index + 1}`} className="w-24 h-24 object-cover mr-2" />
//               ))}
//             </div>
//           </div>
//         )}
...
const AddProductModal = ({ isOpen, onClose, onSubmit }) => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      discount: Yup.number().min(0, 'Discount cannot be negative').max(100, 'Discount cannot exceed 100%'),
      description: Yup.string().required('وارد کردن توضیحات الزامی است'),
      images: Yup.array().min(1, 'بار گزاری حداقل یک عکس الزامی است') // تغییر به آرایه
    }),
    onSubmit: (values) => {
  ...
  const handleImageChange = (e) => {
    ...
    // اضافه کردن فایل‌ها به Formik
    formik.setFieldValue('images', files);
  
    // ایجاد پیش‌نمایش برای هر تصویر
